...
const appReducer = (state = initState, action) => {
  if (action.type === 'ADD_ADVANCE_ANALYTICS') {
    ...
    // console.log('ADD_ADVANCE_ANALYTICS');
  ...
  } else if (action.type === 'SET_COMPANY_PRODUCTS') {
    const newCompanyProducts = {
      ...state.companyProducts,
      [action.companyId]: action.companyProducts
    };
    return { ...state, companyProducts: newCompanyProducts };
  } else {
    return state;
  }
};
